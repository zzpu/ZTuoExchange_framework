...
public class OrderController extends BaseController{
   ...
   /* private static Logger logger = LoggerFactory.getLogger(OrderController.class);*/
    ...
    public MessageResult preOrderInfo(long id) {
        Advertise advertise = (Advertise) advertiseService.findById(id);
    ...
    public MessageResult buy(long id, long coinId, BigDecimal price, BigDecimal money,
        ...
        if (mode == 0) {
            isTrue(isEqual(money.divide(price,8,BigDecimal.ROUND_HALF_UP), amount), msService.getMessage("NUMBER_ERROR"));
        } else {
    ...
    public MessageResult sell(long id, long coinId, BigDecimal price, BigDecimal money,
        ...
        isTrue(compare(advertise.getMaxLimit(), money), msService.getMessage("MONEY_MAX") + advertise.getMaxLimit().toString() + " CNY");
        //获取手续费比例
        MemberGrade grade = gradeService.findOne(user.getMemberGradeId());
        log.info("会员的等级grade={}",grade);
        //计算手续费
        BigDecimal commission = mulRound(amount, grade.getExchangeFeeRate());
        ...
        for(String payMode:payModes){
            if (ALIPAY.getCnName().equals(payMode)&&member.getAlipay()!=null) {
                flag=false;
            } else if (WECHAT.getCnName().equals(payMode)&&member.getWechatPay()!=null) {
                flag=false;
            } else if (BANK.getCnName().equals(payMode)&&member.getBankInfo()!=null) {
                flag=false;
            }
        }
        if(flag){
        ...
        //冻结用户余额=卖出数量+手续费
        if (!(otcWalletService.freezeBalance(wallet, add(amount,commission)).getCode() == 0)) {
            throw new InformationExpiredException("Information Expired");
        }
        Order order1 = orderService.saveOrder(order);
        if (order1 != null) {
    ...
    private MessageResult sendSms(Advertise advertise,String userName) throws Exception {
        ...
        if(driverName.equalsIgnoreCase("two_five_three")){
            result=smsProvider.sendSingleMessage(country.getAreaCode()+member.getMobilePhone(), String.format("您好，你的编号为%s的广告有新的订单，对方账号为%s，请登录系统及时处理",advertise.getId(),userName));
        }else{
            if (country.getAreaCode().equals("86")) {
                result = smsProvider.sendSingleMessage(member.getMobilePhone(), String.format("您好，你的编号为%s的广告有新的订单，对方账号为%s，请登录系统及时处理",advertise.getId(),userName));
            } else {
                result = smsProvider.sendSingleMessage(country.getAreaCode()+member.getMobilePhone(), String.format("您好，你的编号为%s的广告有新的订单，对方账号为%s，请登录系统及时处理",advertise.getId(),userName));
            }
        }
