...
public class AideController extends BaseController {
    ...
    public MessageResult sysHelp(@RequestParam(value = "sysHelpClassification", required = false) SysHelpClassification sysHelpClassification) {
        ...
        if (sysHelpClassification == null) {
            list = sysHelpService.findAllByStatusNotAndSort();
        } else {
    ...
    public MessageResult sysHelp(@PathVariable(value = "id") long id) {
        ...
        SysHelp sysHelp = (SysHelp) sysHelpService.findById(id);
        MessageResult result = MessageResult.success();
    ...
    public MessageResult sysAllHelp(@RequestParam(value = "total",defaultValue = "6")int total){
        ...
        List<JSONObject> result = (List<JSONObject>) redisUtil.get(SysConstant.SYS_HELP);
        if (result != null){
            ...
            redisUtil.set(SysConstant.SYS_HELP,jsonResult,SysConstant.SYS_HELP_EXPIRE_TIME, TimeUnit.SECONDS);
    ...
    public MessageResult sysHelpCate(@RequestParam(value = "pageNo",defaultValue = "1")int pageNo,
        ...
        JSONObject result = (JSONObject) redisUtil.get(SysConstant.SYS_HELP_CATE+cate);
        if (result != null){
            return success(result);
        }else {
            ...
            redisUtil.set(SysConstant.SYS_HELP_CATE+cate,jsonObject,SysConstant.SYS_HELP_CATE_EXPIRE_TIME, TimeUnit.SECONDS);
    ...
    public MessageResult sysHelpTop(@RequestParam(value = "cate")String cate){
        List<SysHelp> result = (List<SysHelp>) redisUtil.get(SysConstant.SYS_HELP_TOP+cate);
        if ( result != null && !result.isEmpty()){
            return success(result);
        }else {
            List<SysHelp> sysHelps = sysHelpService.getgetCateTops(cate);
            redisUtil.set(SysConstant.SYS_HELP_TOP+cate,sysHelps,SysConstant.SYS_HELP_TOP_EXPIRE_TIME,TimeUnit.SECONDS);
